...
from leapfrogai_api.backend.types import (CreateThreadRequest, ModifyThreadRequest, ModifyMessageRequest,
...
def create_message(create_thread):
    ...
    request = CreateMessageRequest(
        role="user",
        content=[TextContentBlock(text=Text(value="test", annotations=[]), type="text")],
        attachments=None,
        metadata={},
    )

    message = threads_client.post(
        f"/openai/v1/threads/{create_thread.json()['id']}/messages", json=request.model_dump()
    )
